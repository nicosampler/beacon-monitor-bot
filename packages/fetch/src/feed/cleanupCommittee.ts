...
export async function cleanupCommittee(logger: CustomLogger) {
  ...
  try {
    ...
    if (lsu.hourlyValidatorStats) {
      const maxSlot =
        getSlotNumberFromTimestamp(lsu.hourlyValidatorStats.getTime()) - 10; // 10 is some buffer just to be safe
      logger.info(`Deleting slots lower than ${maxSlot}`);

      if (maxSlot) {
        const result =
          await prisma.$executeRaw`DELETE FROM "Committee" WHERE slot < ${maxSlot}`;
        totalDeleted += result;
      }
    }
