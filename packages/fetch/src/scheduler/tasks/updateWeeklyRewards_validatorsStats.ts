    ...
    await prisma.$executeRaw`
      ...
      cl_attestation_rewards AS (
        SELECT 
          ...
          COALESCE(SUM(CAST(inactivity AS BIGINT)), 0) as weekly_cl_rewards
      ...
      cl_block_and_sync_rewards AS (
        SELECT 
          ...
          COALESCE(SUM(CAST("blockReward" AS BIGINT)), 0) as weekly_cl_rewards
        FROM "DailyValidatorStats"
        WHERE date >= CURRENT_DATE - INTERVAL '7 days'
        GROUP BY "validatorIndex"
      ),
