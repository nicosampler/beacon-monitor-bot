...
export function getAllUserIds_db(userId?: number) {
  return prisma.user.findMany({
    where: userId ? { id: userId } : { hasBlockedBot: false },
    select: {
      id: true,
      userId: true,
      username: true,
...
export async function deleteUser_db(userId: number) {
  try {
    await prisma.$transaction([
      // Delete from junction tables directly
      prisma.$executeRaw`DELETE FROM "_UserToValidator" WHERE "A" = ${userId}`,
      prisma.$executeRaw`DELETE FROM "_UserToWithdrawalAddress" WHERE "A" = ${userId}`,
      prisma.$executeRaw`DELETE FROM "_FeeRewardAddressToUser" WHERE "B" = ${userId}`,
      // Delete the user
      prisma.user.delete({
        where: { id: userId },
      }),
...
export function setUserBlockedBot_db(chatId: number) {
  return prisma.user.update({
    where: { chatId: chatId },
...
export function setUserUnblockedBot_db(chatId: number) {
  return prisma.user.update({
    where: { chatId: chatId },
