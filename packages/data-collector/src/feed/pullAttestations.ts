...
export const pullAttestations = async (slotNumber: number) => {
  ...
  try {
    ...
    for (const attestation of filteredAttestations) {
      const updates = await processAttestation(slotNumber, attestation, logger);
      allUpdates.push(...updates);
    }

    //await prisma.$transaction(async (tx) => {
    logger.info(`Processing ${allUpdates.length} updates.`);
    ...
    logger.info(`updating slot ${slotNumber} to attestationsFetched: true`);
...
async function processAttestation(
  ...
  logger.info(
    `Processed ${validators.length} committees for attestation in slot ${attestation.data.slot}`
  );
